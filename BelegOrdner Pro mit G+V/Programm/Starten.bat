...
echo    ==============================================================
echo.
echo         B E L E G O R D N E R   P R O
echo.
...
set "SRVSTATUS=NONE"
set "SRVROOT="
for /f "usebackq delims=" %%S in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$mein = '%PROG%'.TrimEnd([char]92); try { $r = (Invoke-RestMethod 'http://localhost:8043/serverinfo' -TimeoutSec 2).root; if ($r -and ($r.TrimEnd([char]92) -ieq $mein)) { 'OK' } else { 'ANDER' } } catch { 'NONE' }"`) do set "SRVSTATUS=%%S"
...
if defined BROWSER (
    start "" "%BROWSER%" "http://localhost:8043/FinanzBuch.html"
) else (
    echo             Chrome nicht gefunden - Standardbrowser wird genutzt
    start "" "http://localhost:8043/FinanzBuch.html"
)
